...
NOSE_ScaleCP <- function(y,### data
                         minD = 2, ### minimum distance
                         L = 25, ### truncation number
                        nchains = 4, ### MCMC chains
  ...
  CP_detect <- function(mcmc_res, minD){
    ...
    res.sim <- apply(res.samples, 2, posterior.mode) ### pointwise MAP
    step.0 <- diff(res.sim)
   ...
   sd_diff <- sd(step.0[which(step.0 > quantile(step.0, .0005) & step.0 < quantile(step.0, .9995))])
    ...
    mu.samples <- mymodel.samples[, grep("^mu",paste0("^",colnames(mymodel.samples), sep=""), fixed=T)]
    mu.est <- mean(mu.samples)
  
    result.cp <- list("Number of CPs" = count.1,
                      "location" = cp_loc, "piecewise sd" = exp(cp_sim),  
                      "mean estimate" = mu.est)
  ...
  NOSErun <- function(seed, data, code, para){
    ...
    NimbleInits <- list(mu = 0, alpha = 1, p = rbeta(L, 1, 1), 
                        v0 = rcauchy(L, 0, 1))
    ...
    #### RJMCMC
    NOSEconf$addMonitors(c("mu", "res", 'z', 'v0'))
